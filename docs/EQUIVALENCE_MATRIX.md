...
| `packages/cli/src/config/config.ts` (approval/non-interactive) | `src/py_agent_runtime/cli/main.py` | done | CLI supports `chat`, `run`, `mode`, `plan enter`, `plan exit` with approval-mode + non-interactive controls and provider retry backoff knobs | CLI-001 |
