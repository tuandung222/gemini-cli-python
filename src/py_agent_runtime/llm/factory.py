...
def create_provider(provider: str, *, model: str | None = None) -> LLMProvider:
    ...
    if normalized == "openai":
        return OpenAIChatProvider(model=model or "gpt-4.1-mini")
    if normalized == "gemini":
        return GeminiChatProvider(model=model or "gemini-2.5-pro")
    if normalized == "anthropic":
        return AnthropicChatProvider(model=model or "claude-3-7-sonnet-latest")
